...
class TorquePredNN(TorquePred):
    def __init__(self, model_path, history = 8, architecture = 'lstm'):
    ...
    def create_xy(self, df):
        ...
        if self.architecture == 'lstm':
            X = self.np.array(df[['x', 'dx']]).astype('float')
            temp = self.np.array([[0, 0] for i in range(h - 1)])
            X = self.np.concatenate((temp, X))
            X = self.np.array([[X[i - j] for j in range(h - 1, -1, -1)] for i in range(h - 1, len(X))])
            y = self.np.array(df[['torque']]).astype('float')
            ...
            X = self.np.array([[[X[i - j][k] for j in range(h - 1, -1, -1)] for k in range(2)] for i in range(h - 1, len(X))])
            y = self.np.array(df[['torque']]).astype('float')
